...
import {showServerPopup} from '../../Store/Actions/auth';
    ...
    componentDidMount() {
        ...
        fetch(this.props.fetchData).then(res => res.json()).then((result) =>
            this.setState({ icons: result })).then(() =>
                this.setMaxCount()).catch(() => {this.Alert("Wystapił błąd."); this.setState({
                    loaded: true
                })}); 
    ...
    setMaxCount = () => {
        if (this.state.icons.length > 0) {
           ...
           // this.setState({ mainTitle: this.state.songs[this.state.songs.length - 1].title })
            ...
            this.setState({ ytID: 'QRi3ULhyQq0',  loaded: true });
        }
...
// zapis do bazy zawsze z vh i vw - przy dodaniu ikony brac z bazy, przy aktualizacji pozycji przeliczac z px na vh vw
            ...
            <div id="prop" class="switch" style={{ position: 'fixed', right: '130px', bottom: '6px', zIndex: '300' }} > <i class="icon-cog" />
                <div id="propField"  >
                    <p>Jasność ikon:</p>
